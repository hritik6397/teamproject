...
export class SellerRegisterComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private sellerAccountService: SellerAccountService, private router: Router, private toastr: ToastrService) { }

  ngOnInit(): void {
    this.createSellerRegisterForm();
  }
  ...
  onSubmit(){
    this.sellerAccountService.sellerregister(this.registerForm.value).subscribe(response => {
    ...
    return control => {
      return timer(500).pipe(
        switchMap(() => {
          ...
          return this.sellerAccountService.checkEmailExists(control.value).pipe(
            map(res => {
